...
def run_test(name, func):
    ...
    try:
        ...
        PASS += 1
        RESULTS.append(
            {"name": name, "status": "PASS", "time": f"{elapsed:.1f}s", "error": None}
        )
...
def test_01_disease_resolution_nsclc():
    ...
    assert "non-small cell" in top.get("name", "").lower(), (
        f"Name mismatch: {top.get('name')}"
    )
...
def test_05_disease_resolution_ols_fallback():
    ...
    top = terms[0]
    assert "EFO_0003060" in top.get("short_form", ""), (
        f"Expected EFO_0003060, got {top.get('short_form')}"
    )
...
def test_06_gene_resolution_egfr():
    ...
    assert top.get("entrezgene") == 1956 or str(top.get("entrezgene")) == "1956", (
        f"Wrong Entrez ID: {top.get('entrezgene')}"
    )
    ...
    print(
        f"       Gene resolved: EGFR -> {ensembl_id}, Entrez: {top.get('entrezgene')}"
    )
...
def test_11_fda_pharmacogenomic_biomarkers():
    ...
    assert len(biomarkers) > 10, (
        f"Expected >10 biomarkers returned, got {len(biomarkers)}"
    )
    ...
    print(
        f"       FDA biomarkers: {total_count} total, {len(biomarkers)} returned, {len(egfr_entries)} EGFR entries"
    )
...
def test_12_search_trials_disease_nsclc():
    ...
    result = tu.tools.search_clinical_trials(
        condition="non-small cell lung cancer", query_term="EGFR mutation", pageSize=5
    )
    ...
    assert "brief_title" in study, "Missing 'brief_title'"
    assert "overall_status" in study, "Missing 'overall_status'"
    print(
        f"       Found {len(studies)} NSCLC/EGFR trials, total: {result.get('total_count', '?')}"
    )
...
def test_13_search_trials_biomarker_kras_g12c():
    ...
    tu = load_tu()
    result = tu.tools.search_clinical_trials(
        condition="colorectal cancer", query_term="KRAS G12C", pageSize=5
    )
...
def test_14_search_trials_biomarker_braf_v600e():
    ...
    tu = load_tu()
    result = tu.tools.search_clinical_trials(
        condition="melanoma", query_term="BRAF V600E", pageSize=5
    )
    assert not isinstance(result, str), f"Got string result: {result}"
    studies = result.get("studies", [])
...
def test_15_search_trials_her2_breast():
    ...
    tu = load_tu()
    result = tu.tools.search_clinical_trials(
        condition="breast cancer", query_term="HER2 positive", pageSize=5
    )
    assert not isinstance(result, str), f"Got string result: {result}"
    studies = result.get("studies", [])
...
def test_16_search_trials_ntrk_basket():
    ...
    tu = load_tu()
    result = tu.tools.search_clinical_trials(query_term="NTRK fusion", pageSize=5)
    assert not isinstance(result, str), f"Got string result: {result}"
    studies = result.get("studies", [])
...
def test_19_search_pagination():
    ...
    result1 = tu.tools.search_clinical_trials(
        condition="lung cancer", query_term="EGFR", pageSize=3
    )
    ...
    print(
        f"       First page: {len(studies1)} trials, total: {total}, has next: {next_token is not None}"
    )
...
def test_20_get_trial_details():
    ...
    result = tu.tools.clinical_trials_get_details(
        action="get_study_details", nct_id="NCT02841579"
    )
    assert isinstance(result, dict), f"Expected dict, got {type(result)}"
    ...
    print(
        f"       Got details for {result.get('nctId')}: {result.get('title', '')[:80]}..."
    )
...
def test_21_get_eligibility_criteria():
    ...
    result = tu.tools.get_clinical_trial_eligibility_criteria(
        nct_ids=["NCT04765059", "NCT02841579"], eligibility_criteria="all"
    )
    assert isinstance(result, list), f"Expected list, got {type(result)}"
    assert len(result) >= 1, f"Expected >= 1 results, got {len(result)}"
    for item in result:
        assert "NCT ID" in item, "Missing 'NCT ID'"
        assert "eligibility_criteria" in item, "Missing 'eligibility_criteria'"
        criteria = item.get("eligibility_criteria", "")
        assert len(criteria) > 50, f"Eligibility text too short: {len(criteria)} chars"
        assert "Inclusion Criteria" in criteria or "inclusion" in criteria.lower(), (
            "No inclusion criteria found"
        )
    print(
        f"       Got eligibility for {len(result)} trials, avg length: {sum(len(r.get('eligibility_criteria', '')) for r in result) // len(result)} chars"
    )
...
def test_22_get_conditions_and_interventions():
    ...
    result = tu.tools.get_clinical_trial_conditions_and_interventions(
        nct_ids=["NCT04765059"], condition_and_intervention="all"
    )
    assert isinstance(result, list), f"Expected list, got {type(result)}"
    assert len(result) > 0, "No results"
    item = result[0]
    assert "condition" in item, f"Missing 'condition'. Keys: {list(item.keys())}"
    assert "interventions" in item, "Missing 'interventions'"
    interventions = item.get("interventions", [])
    assert len(interventions) > 0, "No interventions found"
    for inv in interventions:
        assert "name" in inv, "Missing intervention name"
        assert "type" in inv, "Missing intervention type"
    print(
        f"       Trial has {len(interventions)} interventions, {len(item.get('arm_groups', []))} arm groups"
    )
...
def test_23_get_trial_locations():
    ...
    result = tu.tools.get_clinical_trial_locations(
        nct_ids=["NCT04765059"], location="all"
    )
    assert isinstance(result, list), f"Expected list, got {type(result)}"
    assert len(result) > 0, "No results"
    item = result[0]
...
def test_24_get_trial_status():
    ...
    result = tu.tools.get_clinical_trial_status_and_dates(
        nct_ids=["NCT04765059"], status_and_date="all"
    )
    assert isinstance(result, list), f"Expected list, got {type(result)}"
    assert len(result) > 0, "No results"
    item = result[0]
    assert "overall_status" in item, "Missing 'overall_status'"
    assert item.get("overall_status") is not None, "Status is None"
    print(
        f"       Status: {item.get('overall_status')}, Start: {item.get('start_date')}, Completion: {item.get('completion_date')}"
    )
...
def test_25_get_trial_descriptions():
    ...
    result = tu.tools.get_clinical_trial_descriptions(
        nct_ids=["NCT04765059"], description_type="full"
    )
    assert isinstance(result, list), f"Expected list, got {type(result)}"
    assert len(result) > 0, "No results"
    item = result[0]
    assert "brief_title" in item, "Missing 'brief_title'"
    assert "official_title" in item, "Missing 'official_title'"
    assert "brief_summary" in item, "Missing 'brief_summary'"
...
def test_26_eligibility_biomarker_parsing():
    ...
    * Known ALK rearrangement"""
    ...
    exclusion = (
        sample_text.split("Exclusion Criteria")[1]
        if "Exclusion Criteria" in sample_text
        else ""
    )
...
def test_27_molecular_match_scoring():
    ...
    trial_with_exact = {
        "required_biomarkers": [
            {"gene": "EGFR", "context": "EGFR mutation L858R or exon 19 deletion"}
        ],
        "excluded_biomarkers": [],
    }
    ...
    required_genes = {
        b["gene"].upper() for b in trial_with_exact["required_biomarkers"]
    }
...
def test_28_drug_resolution_osimertinib():
    ...
    tu = load_tu()
    result = tu.tools.OpenTargets_get_drug_id_description_by_name(
        drugName="osimertinib"
    )
    hits = result.get("data", {}).get("search", {}).get("hits", [])
    assert len(hits) > 0, "No drug hits returned"
    top = hits[0]
    assert top.get("id") == "CHEMBL3353410", (
        f"Expected CHEMBL3353410, got {top.get('id')}"
    )
    assert "approved" in top.get("description", "").lower(), (
        "Drug not described as approved"
    )
...
def test_29_drug_mechanism_egfr():
    ...
    tu = load_tu()
    result = tu.tools.OpenTargets_get_drug_mechanisms_of_action_by_chemblId(
    ...
    print(
        f"       Osimertinib targets: {list(set(all_targets))}, MoA: {moa[0].get('mechanismOfAction')}"
    )
...
def test_33_drugs_by_target_egfr():
    ...
    print(
        f"       EGFR drugs: {len(rows)} total (showing first 10), {len(approved)} approved"
    )
...
def test_35_drugbank_targets():
    ...
    assert any("growth factor receptor" in n.lower() for n in target_names), (
        f"EGFR not in targets: {target_names}"
    )
...
def test_36_fda_indications_osimertinib():
    ...
    assert "NSCLC" in ind_text or "non-small cell" in ind_text.lower(), (
        "NSCLC not in indication text"
    )
    assert "EGFR" in ind_text, "EGFR not in indication text"
    print("       FDA indication confirmed for osimertinib in EGFR+ NSCLC")
...
def test_37_pubmed_evidence():
    ...
    result = tu.tools.PubMed_search_articles(
        query="EGFR L858R osimertinib NSCLC clinical trial", max_results=3
    )
    articles = result if isinstance(result, list) else result.get("articles", [])
    assert len(articles) > 0, "No PubMed articles found"
    article = articles[0]
    assert "pmid" in article or "PMID" in article, (
        f"Missing PMID. Keys: {list(article.keys())}"
    )
    assert "title" in article or "Title" in article, "Missing title"
...
def test_40_pharmgkb_egfr():
    ...
    print(
        f"       PharmGKB: EGFR ({gene.get('id')}), chr{gene.get('chr', {}).get('name', '?')}"
    )
...
def test_41_location_analysis():
    ...
    print(
        f"       {len(locations)} sites, {len(countries)} countries, {len(us_sites)} US sites in {len(us_states)} states"
    )
...
def test_43_basket_trial_search():
    ...
    result = tu.tools.search_clinical_trials(query_term="NTRK solid tumor", pageSize=5)
    if isinstance(result, str):
        # Fallback: try even simpler query
        result = tu.tools.search_clinical_trials(query_term="NTRK", pageSize=5)
    assert not isinstance(result, str), "No results for any NTRK query"
    studies = result.get("studies", [])
...
def test_44_expanded_access_search():
    ...
    tu = load_tu()
    result = tu.tools.search_clinical_trials(
        query_term="expanded access osimertinib", pageSize=5
    )
    # May return string if no results, that's OK for expanded access
    if isinstance(result, str):
        print("       No expanded access found (expected for some drugs)")
    else:
        studies = result.get("studies", [])
...
def test_46_end_to_end_egfr_nsclc():
    ...
    disease = tu.tools.OpenTargets_get_disease_id_description_by_name(
        diseaseName="non-small cell lung cancer"
    )
    ...
    elig = tu.tools.get_clinical_trial_eligibility_criteria(
        nct_ids=[nct_id], eligibility_criteria="all"
    )
    assert isinstance(elig, list) and len(elig) > 0, "No eligibility"

    print(
        f"       End-to-end: {efo_id} + {ensembl_id} -> {len(studies)} trials -> {nct_id} with eligibility"
    )
...
def test_48_end_to_end_kras_crc():
    ...
    print(
        f"       End-to-end: CRC + KRAS G12C -> {len(studies)} trials, sotorasib: {hits[0].get('id')}"
    )
...
def test_49_end_to_end_her2_breast():
    ...
    # Search
    trials = tu.tools.search_clinical_trials(
        condition="breast cancer", query_term="HER2 positive amplification", pageSize=5
    )
    assert not isinstance(trials, str), "No trials"
    studies = trials.get("studies", [])
    ...
    print(
        f"       End-to-end: breast + HER2 -> {len(studies)} trials, trastuzumab: {hits[0].get('id')}"
    )
...
def test_50_end_to_end_ntrk_fusion():
    ...
    # Search
    trials = tu.tools.search_clinical_trials(query_term="NTRK fusion", pageSize=5)
    assert not isinstance(trials, str), "No trials"
    studies = trials.get("studies", [])
    assert len(studies) > 0, "No NTRK fusion trials"

    # Drug check
    drug = tu.tools.OpenTargets_get_drug_id_description_by_name(
        drugName="larotrectinib"
    )
    hits = drug.get("data", {}).get("search", {}).get("hits", [])
    ...
    print(
        f"       End-to-end: NTRK fusion -> {len(studies)} trials, larotrectinib: {hits[0].get('id')}"
    )
...
def test_51_rare_biomarker():
    ...
    tu = load_tu()
    result = tu.tools.search_clinical_trials(
        query_term="FGFR3 S249C bladder cancer", pageSize=5
    )
    # May or may not find trials - that's OK
    if isinstance(result, str):
        print("       Rare biomarker: No trials found (expected)")
    else:
        studies = result.get("studies", [])
...
def test_52_no_molecular_criteria():
    ...
    tu = load_tu()
    result = tu.tools.search_clinical_trials(
        condition="pancreatic cancer", query_term="pancreatic cancer", pageSize=5
    )
...
def main():
    ...
    run_test(
        "03. Disease resolution: colorectal", test_03_disease_resolution_colorectal
    )
    run_test(
        "04. Disease resolution: breast cancer",
        test_04_disease_resolution_breast_cancer,
    )
    run_test(
        "05. Disease resolution: OLS fallback", test_05_disease_resolution_ols_fallback
    )
    ...
    run_test(
        "11. FDA pharmacogenomic biomarkers", test_11_fda_pharmacogenomic_biomarkers
    )
    ...
    run_test(
        "13. Search trials: CRC/KRAS G12C", test_13_search_trials_biomarker_kras_g12c
    )
    run_test(
        "14. Search trials: melanoma/BRAF V600E",
        test_14_search_trials_biomarker_braf_v600e,
    )
    ...
    run_test(
        "17. Search trials: intervention (osimertinib)",
        test_17_search_trials_intervention,
    )
    run_test(
        "18. Alternative search (clinical_trials_search)", test_18_alternative_search
    )
    ...
    run_test(
        "22. Get conditions and interventions", test_22_get_conditions_and_interventions
    )
    ...
    run_test(
        "32. Drug resolution: larotrectinib", test_32_drug_resolution_larotrectinib
    )
