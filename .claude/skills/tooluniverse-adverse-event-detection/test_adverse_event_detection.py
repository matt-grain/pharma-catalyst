...
def test_01_drug_disambiguation_atorvastatin():
    ...
    result = tu.tools.OpenTargets_get_drug_chembId_by_generic_name(
...
def test_02_drug_blackbox_status():
    ...
    result = tu.tools.OpenTargets_get_drug_blackbox_status_by_chembl_ID(
        chemblId="CHEMBL1487"
    )
    assert isinstance(result, dict), f"Expected dict, got {type(result)}"
    ...
    assert drug["blackBoxWarning"] is False, (
        "Atorvastatin should not have blackbox warning"
    )
...
def test_03_drug_mechanism_of_action():
    ...
    moa = rows[0]
    assert "HMG-CoA reductase" in moa["mechanismOfAction"], (
        f"Unexpected MOA: {moa['mechanismOfAction']}"
    )
    assert moa["actionType"] == "INHIBITOR", (
        f"Expected INHIBITOR, got {moa['actionType']}"
    )
...
def test_05_faers_top_reactions():
    ...
    # Verify structure
    first = result[0]
    assert "term" in first, "Missing 'term' key"
    assert "count" in first, "Missing 'count' key"
    assert isinstance(first["count"], int), (
        f"Count should be int, got {type(first['count'])}"
    )
    # Verify myalgia is present (known statin AE)
    terms = [r["term"].upper() for r in result]
    assert any("MYALGIA" in t for t in terms), (
        f"Expected MYALGIA in results, got top terms: {terms[:10]}"
    )
...
def test_06_faers_seriousness():
    ...
    result = tu.tools.FAERS_count_seriousness_by_drug_event(
        medicinalproduct="ATORVASTATIN"
    )
    assert isinstance(result, list), f"Expected list, got {type(result)}"
    assert len(result) >= 2, "Expected at least 2 categories (Serious/Non-serious)"
    ...
    print(
        f"  Serious %: {terms['Serious'] / (terms['Serious'] + terms['Non-serious']) * 100:.1f}%"
    )


def test_07_faers_outcomes():
    """Phase 1: Get outcome distribution."""
    result = tu.tools.FAERS_count_outcomes_by_drug_event(
        medicinalproduct="ATORVASTATIN"
    )
    assert isinstance(result, list), f"Expected list, got {type(result)}"
    terms = {r["term"]: r["count"] for r in result}
...
def test_08_faers_age_distribution():
    ...
    result = tu.tools.FAERS_count_patient_age_distribution(
        medicinalproduct="ATORVASTATIN"
    )
    assert isinstance(result, list), f"Expected list, got {type(result)}"
    terms = {r["term"]: r["count"] for r in result}
...
def test_10_faers_reporter_country():
    ...
    result = tu.tools.FAERS_count_reportercountry_by_drug_event(
        medicinalproduct="ATORVASTATIN"
    )
    assert isinstance(result, list), f"Expected list, got {type(result)}"
...
def test_11_disproportionality_rhabdomyolysis():
    ...
    assert result.get("status") == "success", (
        f"Status not success: {result.get('status')}"
    )
    ...
    assert "Strong" in sd["signal_strength"], (
        f"Expected strong signal, got {sd['signal_strength']}"
    )

    print(
        f"  PRR: {prr:.3f} (CI: {metrics['PRR']['ci_95_lower']:.3f}-{metrics['PRR']['ci_95_upper']:.3f})"
    )
    print(
        f"  ROR: {ror:.3f} (CI: {metrics['ROR']['ci_95_lower']:.3f}-{metrics['ROR']['ci_95_upper']:.3f})"
    )
    print(
        f"  IC:  {ic:.3f} (CI: {metrics['IC']['ci_95_lower']:.3f}-{metrics['IC']['ci_95_upper']:.3f})"
    )
...
def test_13_serious_events_death():
    ...
    print(
        f"  Top 3: {[(r['reaction'], r['count']) for r in result['top_serious_reactions'][:3]]}"
    )
...
def test_14_demographic_stratification():
    ...
    for s in strat:
        sex_label = {0: "Unknown", 1: "Male", 2: "Female"}.get(
            s["group"], f"Code {s['group']}"
        )
...
def test_15_meddra_rollup():
    ...
    result = tu.tools.FAERS_rollup_meddra_hierarchy(
        operation="rollup_meddra_hierarchy", drug_name="ATORVASTATIN"
    )
    assert isinstance(result, dict), f"Expected dict, got {type(result)}"
    assert result.get("status") == "success", "Status not success"
...
def test_17_fda_boxed_warning_present():
    ...
    if "results" in result and len(result["results"]) > 0:
        ...
        if bw:
            ...
            print(
                "  No boxed_warning field in results (metformin may have changed labels)"
            )
    else:
...
def test_18_fda_contraindications():
    ...
    assert "liver" in text or "hypersensitivity" in text, (
        f"Expected liver/hypersensitivity, got: {text[:200]}"
    )
...
def test_22_target_safety_profile():
    ...
    result = tu.tools.OpenTargets_get_target_safety_profile_by_ensemblID(
        ensemblId="ENSG00000113161"
    )
    assert isinstance(result, dict), f"Expected dict, got {type(result)}"
    target = result["data"]["target"]
    assert target["approvedSymbol"] == "HMGCR", (
        f"Wrong target: {target['approvedSymbol']}"
    )
...
def test_23_admet_toxicity():
    ...
    if isinstance(result, dict):
        ...
        if len(result) > 0:
            print(
                f"  First item keys: {list(result[0].keys())[:10] if isinstance(result[0], dict) else 'N/A'}"
            )
...
def test_25_aggregate_class_reactions():
    ...
    terms = [r["term"].upper() for r in result]
    assert any("MYALGIA" in t for t in terms), "Expected MYALGIA in class aggregate"
...
def test_31_pubmed_search():
    ...
    result = tu.tools.PubMed_search_articles(
        query="atorvastatin adverse events safety rhabdomyolysis", limit=5
    )
    assert isinstance(result, list), f"Expected list, got {type(result)}"
    ...
    for article in result[:3]:
        print(
            f"    - [{article['pmid']}] {article['title'][:80]}... ({article['pub_year']})"
        )
...
def test_32_openalex_search():
    ...
    result = tu.tools.openalex_search_works(
        query="atorvastatin statin safety adverse events", limit=5
    )
...
def test_35_pembrolizumab_disproportionality():
    ...
    assert result.get("status") == "success"
    prr = result["metrics"]["PRR"]["value"]
    assert prr > 1.0, "Expected PRR > 1 for pembrolizumab colitis"
    print(f"  PRR: {prr:.3f}")
...
def test_37_ibuprofen_gi_bleeding():
    ...
    sd = result["signal_detection"]
    assert sd["signal_detected"] is True, (
        "GI hemorrhage should be a signal for ibuprofen"
    )
    print(f"  PRR: {prr:.3f}")
    print(f"  Signal: {sd['signal_strength']}")
...
def test_40_individual_case_reports():
    ...
    result = tu.tools.FAERS_search_adverse_event_reports(
        medicinalproduct="atorvastatin", limit=3
    )
    assert isinstance(result, list), f"Expected list, got {type(result)}"
...
def test_41_search_by_drug_and_reaction():
    ...
    result = tu.tools.FAERS_search_reports_by_drug_and_reaction(
        medicinalproduct="ATORVASTATIN", reactionmeddrapt="Rhabdomyolysis", limit=3
    )
    assert isinstance(result, list), f"Expected list, got {type(result)}"
...
def test_42_temporal_trends():
    ...
    result = tu.tools.FAERS_analyze_temporal_trends(
        operation="analyze_temporal_trends", drug_name="ATORVASTATIN"
    )
    assert isinstance(result, dict), f"Expected dict, got {type(result)}"
...
def test_43_safety_signal_score_calculation():
    ...
    has_contras = (
        isinstance(contras, dict) and contras.get("meta", {}).get("total", 0) > 0
    )
    ...
    papers = tu.tools.PubMed_search_articles(
        query="atorvastatin adverse events safety", limit=5
    )
    ...
    print(
        f"  Label Warning Score: {label_score}/25 (boxed={has_boxed}, contras={has_contras})"
    )
...
if __name__ == "__main__":
    tests = [
        ...
        (
            "Phase 0: Drug disambiguation (atorvastatin -> CHEMBL1487)",
            test_01_drug_disambiguation_atorvastatin,
        ),
        ...
        (
            "Phase 2: Disproportionality - rhabdomyolysis (strong signal)",
            test_11_disproportionality_rhabdomyolysis,
        ),
        ...
        # Phase 3: FDA Label
        (
            "Phase 3: Boxed warning - none (atorvastatin)",
            test_16_fda_boxed_warning_none,
        ),
        (
            "Phase 3: Boxed warning - present (metformin)",
            test_17_fda_boxed_warning_present,
        ),
        ...
        (
            "Phase 5: Aggregate class reactions (statins)",
            test_25_aggregate_class_reactions,
        ),
        ...
        (
            "Phase 6: PharmGKB dosing guidelines (SLCO1B1)",
            test_30_pharmgkb_dosing_guidelines,
        ),
        # Phase 7: Literature
        ...
        # Cross-drug tests
        (
            "Cross-drug: Metformin + lactic acidosis (strong signal)",
            test_33_metformin_disproportionality,
        ),
        ("Cross-drug: Pembrolizumab FAERS profile", test_34_pembrolizumab_reactions),
        (
            "Cross-drug: Pembrolizumab + colitis",
            test_35_pembrolizumab_disproportionality,
        ),
        ...
        # Integration
        (
            "Integration: Safety Signal Score calculation",
            test_43_safety_signal_score_calculation,
        ),
    ]

    print(f"\n{'#' * 70}")
    print("# ADVERSE DRUG EVENT SIGNAL DETECTION - COMPREHENSIVE TEST SUITE")
    print(f"# Tests: {len(tests)}")
    print("# Drugs: Atorvastatin, Metformin, Pembrolizumab, Ibuprofen, Tocilizumab")
    print(f"{'#' * 70}")
