...
def analyze_ddi(tu, drug_a, drug_b):
    ...
    try:
        ...
        if result_a.get("status") == "success":
            ...
            if names_a:
                ...
                print(
                    f"✅ Found: {names_a[0].get('name')} (RxCUI: {names_a[0].get('rxcui')})"
                )
            else:
    ...
    try:
        ...
        if result_b.get("status") == "success":
            ...
            if names_b:
                ...
                print(
                    f"✅ Found: {names_b[0].get('name')} (RxCUI: {names_b[0].get('rxcui')})"
                )
            else:
        ...
        if result.get("status") == "success":
            ...
            for interaction in interactions:
                ...
                if drug_b.lower() in interacting_drug:
                    ...
                    print(
                        f"✅ Found interaction: {interaction.get('description')[:100]}..."
                    )
                    break
        else:
    ...
    try:
        ...
        if result.get("status") == "success":
            ...
            if drugs:
                ...
                report["identifiers"][drug_a]["drugbank_id"] = drug_info.get(
                    "drugbank_id"
                )
        else:
    ...
    try:
        result = tu.tools.FAERS_count_reactions_by_drug_event(
            drug_name=drug_a, event_name="drug interaction"
        )
        ...
        if result.get("status") == "success":
            ...
            print(
                f"✅ Found {count} adverse event reports mentioning '{drug_a}' + 'drug interaction'"
            )
