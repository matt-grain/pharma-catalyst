...
class DDIAnalyzer:
    ...
    def analyze(self, drug_a, drug_b, output_file=None):
        ...
        self._update_report(
            output_file, "## 1. Drug Identification", report["identifiers"]
        )

        # STEP 2: Mechanism Analysis (DrugBank)
        report["mechanisms"] = self._analyze_mechanisms(drug_a, drug_b)
        self._update_report(
            output_file, "## 2. Interaction Mechanisms", report["mechanisms"]
        )
        ...
        self._update_report(
            output_file, "## 4. FDA Label Warnings", report["fda_labels"]
        )
        ...
        self._update_report(
            output_file, "## 6. Post-Market Surveillance", report["clinical_evidence"]
        )
        ...
        self._update_report(
            output_file,
            "## 7. Risk Assessment",
            {"score": report["risk_score"], "severity": report["severity"]},
        )

        # STEP 8: Management Recommendations
        report["recommendations"] = self._generate_recommendations(report)
        self._update_report(
            output_file, "## 8. Clinical Management", report["recommendations"]
        )
    ...
    def _create_report_file(self, filename, drug_a, drug_b):
        ...
        with open(filename, "w") as f:
            f.write("# Drug-Drug Interaction Analysis Report\n\n")
            f.write(f"**Drug Pair**: {drug_a.upper()} + {drug_b.upper()}\n")
            f.write(
                f"**Analysis Date**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
            )
            f.write("**Generated by**: ToolUniverse DDI Pipeline\n\n")
            f.write("---\n\n")
    ...
    def _identify_drugs(self, drug_a, drug_b):
        ...
        try:
            result = self.tu.tools.drugbank_get_drug_basic_info_by_drug_name_or_id(
                query=drug_a, case_sensitive=False, exact_match=False, limit=1
            )
    ...
    def _analyze_mechanisms(self, drug_a, drug_b):
        ...
        try:
            result = self.tu.tools.drugbank_get_drug_interactions_by_drug_name_or_id(
                query=drug_a, case_sensitive=False, exact_match=False, limit=50
            )
            ...
            if result.get("data", {}).get("interactions"):
                ...
                for interaction in interactions:
                    ...
                    if (
                        drug_b.lower() in interacting_drug
                        or interacting_drug in drug_b.lower()
                    ):
                        mechanisms.append(
                            {
        ...
        try:
            result = self.tu.tools.drugbank_get_drug_interactions_by_drug_name_or_id(
                query=drug_b, case_sensitive=False, exact_match=False, limit=50
            )
            ...
            if result.get("data", {}).get("interactions"):
                ...
                for interaction in interactions:
                    interacting_drug = interaction.get("name", "").lower()
                    if (
                        drug_a.lower() in interacting_drug
                        or interacting_drug in drug_a.lower()
                    ):
                        mechanisms.append(
                            {
        ...
        if not mechanisms:
            print("   ℹ️ No direct interactions found in DrugBank")
            mechanisms.append(
                {
                    "direction": "N/A",
            ...
            try:
                result = (
                    self.tu.tools.drugbank_get_pharmacology_by_drug_name_or_drugbank_id(
                        query=drug, case_sensitive=False, exact_match=False, limit=1
                    )
                )
    ...
    def _search_literature(self, drug_a, drug_b):
        ...
        try:
            result = self.tu.tools.PubMed_search_articles(query=query, max_results=10)
        ...
        for drug in [drug_a, drug_b]:
            ...
            try:
                result = self.tu.tools.FAERS_count_reactions_by_drug_event(
                    medicinalproduct=drug, event_name="drug interaction"
                )
    ...
    def _generate_recommendations(self, report):
        ...
        if severity == "MAJOR":
            ...
            recommendations.append(
                "If combination unavoidable, close monitoring required"
            )
...
def main():
    ...
    print("=" * 80)
    print("\n📄 Reports generated:")
    print("   - DDI_report_warfarin_amoxicillin.md")
    print("   - DDI_report_simvastatin_ketoconazole.md")
